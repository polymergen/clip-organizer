        ...
        for i, record in enumerate(records):
            ...
            if ',' in record:
                ...
                if len(parts) == 2:
                    ...
                    # Also check if anagram is part of the filename
                    if anagram in name_without_ext:
                        decoded_name = name_without_ext.replace(anagram, original_name)
                        print(f"✓ PARTIAL MATCH! Decoded anagram: {decoded_name}")
...
def GenerateScreencaps(input_video_file, db):
    ...
    # Loop through the number of rows and columns, capturing a frame from the video
